...
class ProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //cells not being registered
        if isAnArtist {
            postsCollectionView.register(UINib(nibName: "PostCell", bundle: nil), forCellWithReuseIdentifier: "postCell")
        } else {
            postsCollectionView.register(UINib(nibName: "EnthusiastCell", bundle: nil), forCellWithReuseIdentifier: "enthusiast")
        }
    }
        ...
        db.fetchArtist(userID: user.uid){ [weak self] (result) in
            ...
            case.success(let artist1):
                DispatchQueue.main.async {
                    ...
                    self?.isAnArtist = artist1.isAnArtist
                }
            }
        }
            //        profileViewModel.fetchArtist(profileVC: self, user: user)
        ...
        func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            ...
            if collectionView == postsCollectionView {
                ...
                if isAnArtist {
                    ...
                    guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "enthusiast", for: indexPath) as? EnthusiastCell else {
                        fatalError("could not conform to enthusiastCell")
                    }
